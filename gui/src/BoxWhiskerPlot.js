...
const BWGraph = ({ geojsonData }) => {
  ...
  useEffect(() => {
    ...
    const calculateVotePercentages = async (data) => {
      return await Promise.all(data.map(async (d) => {
        ...
        return ({
            district: districtId,
            repPercentage: republicanPercentage,
            demPercentage: democratPercentage
        })
      }));
    };
    ...
    calculateVotePercentages(geojsonData.features).then((percentages) => {
