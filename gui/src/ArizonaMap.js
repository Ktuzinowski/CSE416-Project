...
import { LeftDistrictPanelAZ } from "./LeftDistrictPanelAZ";
import { LeftPrecinctPanelAZ } from "./LeftPrecinctPanelAZ";
...
import arizonaCongressionalData  from "./arizona_data/ArizonaAggDistricts.geojson" 
//this is for aggregated precincts (has census blocks data)
import arizonaPrecinctData from "./arizona_data/ArizonaAggPrecincts.geojson";
...
export const ArizonaMap = () => {
 ...
 // const [utGeo, setGeo] = useState(null);
  ...
  const setStyleForSelection = (feature) => {
    let totalPop = feature.properties.CVAP_TOT20;
  ...
  const styleDistricts = (feature) => {
    if (selectedRace === "") {
      ...
      //if CHOROpleth
      const totalPop = feature.properties.CVAP_TOT20;
      const racePop = feature.properties[selectedRace];
      const percent = totalPop > 0 ? (racePop / totalPop) * 100 : 0;
    ...
    const popupContent = `
        <div style="font-family: Arial, sans-serif; line-height: 1.5;">
            <h3 style="margin: 0;">Precinct: ${
              feature.properties.PRECINCTNA}</h3>
            <p><strong>Population:</strong> ${Math.round(feature.properties.CVAP_TOT20)}</p>
        ...
        <p><strong>White:</strong> ${(
          (feature.properties.CVAP_WHT20 / feature.properties.CVAP_TOT20) *
          100
        ).toFixed(2)}%</p>
        <p><strong>Black:</strong> ${(
          (feature.properties.CVAP_BLK20 / feature.properties.CVAP_TOT20) *
          100
        ).toFixed(2)}%</p>
        <p><strong>Hispanic:</strong> ${(
          (feature.properties.CVAP_HSP20 / feature.properties.CVAP_TOT20) *
          100
        ).toFixed(2)}%</p>
        <p><strong>Asian:</strong> ${(
          (feature.properties.CVAP_ASN20 / feature.properties.CVAP_TOT20) *
          100
        ).toFixed(2)}%</p>
        <p><strong>Pacific:</strong> ${(
          (feature.properties.CVAP_NHP20 / feature.properties.CVAP_TOT20) *
          100
        ).toFixed(2)}%</p>
        <p><strong>Native:</strong> ${(
          (feature.properties.CVAP_AIA20 / feature.properties.CVAP_TOT20) *
          100
        ).toFixed(2)}%</p>
    ...
    <>
      <div className="map-wrapper">
        ...
        <div className="map-container">
          <MapContainer
            center={[34.0489, -113.0937]} //center on arizona coords
            zoom={6}
            minZoom={3}
