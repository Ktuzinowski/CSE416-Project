...
export const SMDBarChartGraph = ({ data }) => {
    const svgRef = useRef();

    useEffect(() => {
        // Data: number of representatives for each district in Utah (4 districts)
        const data = [
            { district: 1, Democrat: 0, Republican: 1 },
            { district: 2, Democrat: 0, Republican: 1 },
            { district: 3, Democrat: 0, Republican: 1 },
            { district: 4, Democrat: 0, Republican: 1 }
        ];
        ...
        .text("Utah SMD Election Results");
        ...
        const y = d3.scaleLinear()
            .domain([0, 1]) // Assuming the max number of reps in any district is 3
            .nice()
            .range([height, 0]);
